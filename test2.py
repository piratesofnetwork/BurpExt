        ...
        if messageIsRequest:
            ...
            for payload in payload_list:
                for parameter in parameters:
                    # Create a new parameter with the payload appended
                    new_value = parameter.getValue() + payload
                    new_param = self.helpers.buildParameter(parameter.getName(), new_value, parameter.getType())
                    
                    # Update the request with the new parameter
                    new_request = self.helpers.updateParameter(messageInfo.getRequest(), new_param)
